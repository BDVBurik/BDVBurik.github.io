(function () {
  //спижжено и порезано...  рейтинг имдб для лампа
  "use strict";
  ...
  function rating_kp_imdb(card) {
    ...
    var kp_prox = "https://worker-patient-dream-26d7.bdvburik.workers.dev/"; //https://cors.kp556.workers.dev:8443/";//
    var params = {
      id: card.id,
    ...
    function _showRating(data) {
      if (data) {
        var kp_rating =
          !isNaN(data.kp) && data.kp !== null
            ? parseFloat(data.kp).toFixed(1)
            : "0.0";
        var imdb_rating =
          !isNaN(data.imdb) && data.imdb !== null
