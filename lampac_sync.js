...
(function () {
    ...
    function buildUrl(path, extra) {
        ...
        url += (url.indexOf('?') === -1 ? '?' : '&') + 'account_email=' + encodeURIComponent(email);
    ...
    function request(method, path, body, onSuccess, onError, extraQuery) {
        ...
        try {
            ...
            if (method === 'POST' && typeof body === 'string' && body.indexOf('=') !== -1) {
            ...
            xhr.ontimeout = function () { if (onError) onError(0, null); };
            xhr.onerror   = function () { if (onError) onError(0, null); };
    ...
    function loadLang() {
        Lampa.Lang.add({
            lampac_sync_title:        { ru: 'Синхронизация (Worker)',       en: 'Worker Sync',           uk: 'Синхронізація (Worker)' },
            lampac_sync_host:         { ru: 'Адрес сервера',                en: 'Server URL',             uk: 'Адреса сервера' },
            lampac_sync_host_hint:    { ru: 'URL вашего Cloudflare Worker', en: 'Your Cloudflare Worker URL', uk: 'URL вашого Worker' },
            lampac_sync_email:        { ru: 'Email аккаунта',               en: 'Account email',          uk: 'Email акаунту' },
            lampac_sync_email_hint:   { ru: 'Должен совпадать с настройкой воркера', en: 'Must match worker ALLOWED_EMAIL', uk: 'Має збігатися з ALLOWED_EMAIL' },
            lampac_sync_timecodes:    { ru: 'Синхронизировать таймкоды',    en: 'Sync timecodes',         uk: 'Синхронізувати таймкоди' },
            lampac_sync_bookmarks:    { ru: 'Синхронизировать закладки',    en: 'Sync bookmarks',         uk: 'Синхронізувати закладки' },
            lampac_sync_debug:        { ru: 'Отладка в консоли',            en: 'Debug console logs',     uk: 'Відладка в консолі' },
            lampac_sync_pull_now:     { ru: 'Загрузить с сервера',          en: 'Pull from server',       uk: 'Завантажити з сервера' },
            lampac_sync_push_now:     { ru: 'Отправить на сервер',          en: 'Push to server',         uk: 'Відправити на сервер' },
            lampac_sync_status_ok:    { ru: 'Сервер доступен ✓',            en: 'Server OK ✓',            uk: 'Сервер доступний ✓' },
            lampac_sync_status_fail:  { ru: 'Ошибка соединения',            en: 'Connection error',       uk: 'Помилка з\'єднання' },
            lampac_sync_status_nokey: { ru: 'Email не задан',               en: 'Email not set',          uk: 'Email не вказано' },
            lampac_sync_pulled:       { ru: 'Данные загружены с сервера',   en: 'Data pulled from server', uk: 'Дані завантажено' },
            lampac_sync_pushed:       { ru: 'Данные отправлены на сервер',  en: 'Data pushed to server',  uk: 'Дані відправлено' },
        });
    }
    ...
    var Timecodes = {
        ...
        add: function (e) {
            ...
            request('POST', '/timecode/add', form, null, null, 'card_id=' + encodeURIComponent(card_id));
        },
        ...
        pullForCurrent: function () {
            ...
            request('GET', '/timecode/all', null, function (json) {
            ...
            }, null, 'card_id=' + encodeURIComponent(card_id));
        },
    ...
    var Bookmarks = {
        ...
        applyFromServer: function (data) {
            ...
            try {
                ...
                Object.keys(data).forEach(function (k) {
                    ...
                    local[k] = data[k].map(function (item) {
                ...
                } else if (Lampa.Favorite && typeof Lampa.Favorite.init === 'function') {
                    Lampa.Favorite.init();
                }
        ...
        pull: function () {
            ...
            var self = this;
            dbg('→ bookmark/list');
            request('GET', '/bookmark/list', null, function (json) {
                if (!json) return;
        ...
        pushFull: function () {
            ...
            request('POST', '/bookmark/set', body, function () {
                dbg('← bookmark/set ok');
            }, function (status) {
        ...
        schedulePush: function () {
            ...
            self.pushTimer = setTimeout(function () { self.pushFull(); }, 1500);
        },
        ...
        sendEvent: function (path, event) {
            ...
            if (card) {
                ...
                payload.card = { id: card.id, title: card.title, name: card.name, poster_path: card.poster_path };
            }
    ...
    var SVG = '<svg width="64" height="64" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8 8 0 0 1-8 8z" fill="white"/><path d="M12 6v6l4 2" stroke="white" stroke-width="2" stroke-linecap="round"/></svg>';

    var statusEl = null;

    function checkServer(cb) {
        if (!getEmail()) {
            if (statusEl) statusEl.find('.settings-param__descr').text(T('lampac_sync_status_nokey'));
            if (cb) cb(false);
            return;
        }
        request('GET', '/timecode/all', null, function () {
            if (statusEl) statusEl.find('.settings-param__descr').text(T('lampac_sync_status_ok'));
            if (cb) cb(true);
        }, function () {
            if (statusEl) statusEl.find('.settings-param__descr').text(T('lampac_sync_status_fail'));
            if (cb) cb(false);
        }, 'card_id=0_movie');
    }
    ...
    whenReady(function () {
        ...
        console.log('[' + PLUGIN_NAME + '] v' + PLUGIN_VER + ' started →', getHost());
    });
