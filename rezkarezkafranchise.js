(function () {
  "use strict";
  let total_pages, secuses;
  let card = [];
  let rezka_domain = "https://rezka.ag/franchises/";
  var cors = "https://worker-patient-dream-26d7.bdvburik.workers.dev/";
  var cache = Lampa.Storage.cache("my_colls", 5000, {});
  var network = new Lampa.Reguest();
  var api_url =
    Lampa.Utils.protocol() + Lampa.Manifest.cub_domain + "/api/collections/";
  function main(params, oncomplite, onerror) {
    network.silent(
      api_url + "list?page=" + params.page,
      function (data) {
  ...
  function datat() {
    network.silent(
      ...
      function (str) {
        ...
        let data = card(str);
        console.log("data = card(str); ", data);
        // this.build(data);
        if (/*object.card*/ 1)
  ...
  card = function (str) {
    ...
    (secuses = true),
      (total_pages = $(".b-navigation", str).find("a:last-child")[0]
        .previousElementSibling.innerText); // $(".b-navigation", str).find("a:last-child").length;
  ...
  function full(params, oncomplite, onerror) {
    // network.silent(
    //   cors + rezka_domain + params.url + "page/" + params.page + "/",
    //   function (data) {
    ...
    {
      let dataq = datat();
      console.log(" let data = datat()"), dataq;
      var dat = card(dataq);
      console.log("  var dat = card(data);"), dat;
      oncomplite(dat);
    }
    //   },
    onerror;
